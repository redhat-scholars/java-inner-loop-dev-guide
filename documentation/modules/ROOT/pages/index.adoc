...
This lab has requires you to have access to an OpenShift cluster. This cluster can be remote or local if you use link:https://developers.redhat.com/products/codeready-containers/overview[*CodeReady Containers*, window=_blank] or *CRC* for short.

NOTE: Use this link:https://cloud.redhat.com/openshift/create/local[link, window=_blank] if you want to jump directly to the instructions to download and install *CRC*.
...
If you add the above variables to this lab url you will end up with something like this: link:{site-url}/{page-component-name}?USERNAME=developer&PASSWORD=developer&CLUSTER_SUBDOMAIN=apps-crc.testing#lab-parameters[{site-url}/{page-component-name}?USERNAME=developer&PASSWORD=developer&CLUSTER_SUBDOMAIN=apps-crc.testing, window=_blank]
